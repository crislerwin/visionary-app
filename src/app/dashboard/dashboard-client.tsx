...
export function DashboardClient() {
    ...
    const { data: monthlyStats, isLoading: statsLoading } =
        api.transaction.getMonthlyStats.useQuery(
    ...
    const cards = useMemo(() => {
        const totalReceitas = compareSeries.reduce(
            (sum, m) => sum + (m.receitas ?? 0),
            0,
        );
        const totalDespesas = compareSeries.reduce(
            (sum, m) => sum + (m.despesas ?? 0),
            0,
        );
        ...
        const lastHalfSaldo =
            balanceSeries[balanceSeries.length - 1]?.saldo ?? 0;
                ...
                <Card className="min-h-0 py-2">
                    ...
                    <CardContent className="px-3 pb-2 pt-0">
                        <ChartContainer
                            ...
                            className="h-[140px] w-full sm:h-[170px]"
                        >
                            <AreaChart
                                data={balanceSeries}
