      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            <code className="rounded bg-muted px-1.5 py-0.5 text-xs">Valor</code> e{" "}
            <code className="rounded bg-muted px-1.5 py-0.5 text-xs">Categoria</code>.
          </CardDescription>
        </CardHeader>
            ...
            {result.errors.length > 0 && (
              <ul className="mt-3 max-h-32 space-y-1 overflow-y-auto rounded border p-2 text-xs">
                {result.errors.map((err, i) => (
                  <li key={`err-${String(i)}`} className="text-destructive">
                    {err}
                  </li>
                ))}
              </ul>
