            ...
            complete: (results) => {
                ...
                for (const fieldMeta of SYSTEM_FIELDS) {
                    const match = hdrs.find((h) =>
                        h.toLowerCase().includes(fieldMeta.key.toLowerCase()),
                    );
    ...
    const handleImport = () => {
        ...
        if (missing.length) {
            setResult({
                ...
                total: rows.length,
                errors: [
                    `Colunas obrigatórias: ${missing.map((m) => m.label).join(", ")}`,
                ],
            });
            return;
        }
    ...
    const previewRows = useMemo(() => {
        return current.map((r, i) => {
            ...
            return {
                ...
                description: columnMapping.description
                    ? (r[columnMapping.description] ?? "")
                    : "",
                category: columnMapping.category
                    ? (r[columnMapping.category] ?? null)
                    : null,
        ...
        <div className="space-y-6">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Envie um arquivo{" "}
                        <span className="font-medium">.csv</span> contendo as
                        colunas:{" "}
                ...
                <Card>
                    <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <CardDescription>
                                Dados extraídos do arquivo enviado.
                            </CardDescription>
                        </div>
            ...
            <ColumnMappingDialog
                ...
                onDateFormatChange={setDateFormat as (v: string) => void}
...
function ColumnMappingDialog({
    ...
    const requiredMissing = SYSTEM_FIELDS.filter(
        (f) =>
            f.required &&
            (!columnMapping[f.key] || columnMapping[f.key] === IGNORE),
    );
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Associe cada coluna detectada no arquivo aos campos
                        correspondentes do sistema. Campos marcados como
