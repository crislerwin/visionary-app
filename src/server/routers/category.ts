...
export const categoryRouter = router({
  ...
  list: tenantProcedure.input(categoryFilterSchema).query(async ({ ctx, input }) => {
    const where = {
  ...
  // Get category by ID
  getById: tenantProcedure.input(getByIdSchema).query(async ({ ctx, input }) => {
  ...
  // Delete category
  delete: tenantProcedure.input(deleteCategorySchema).mutation(async ({ ctx, input }) => {
    const existingCategory = await prisma.category.findFirst({
      where: {
        id: input.id,
        tenantId: ctx.tenantId,
