...
export const agentRouter = router({
  ...
  createConfig: tenantProcedure.input(createAgentConfigSchema).mutation(async ({ ctx, input }) => {
  ...
  updateConfig: tenantProcedure.input(updateAgentConfigSchema).mutation(async ({ ctx, input }) => {
    const config = await prisma.agentConfig.findUnique({
      where: { tenantId: ctx.tenantId },
    });
  ...
  // Delete agent config
  deleteConfig: tenantProcedure.mutation(async ({ ctx }) => {
    const config = await prisma.agentConfig.findUnique({
      where: { tenantId: ctx.tenantId },
    });
  ...
  regenerateWebhookSecret: tenantProcedure.mutation(async ({ ctx }) => {
    const config = await prisma.agentConfig.findUnique({
      where: { tenantId: ctx.tenantId },
    });
