...
const createTransactionSchema = z.object({
  ...
  status: z.enum([TransactionStatus.COMPLETED, TransactionStatus.PENDING, TransactionStatus.CANCELLED]).default(TransactionStatus.COMPLETED),
});
...
const updateTransactionSchema = z.object({
  ...
  status: z.enum([TransactionStatus.COMPLETED, TransactionStatus.PENDING, TransactionStatus.CANCELLED]).optional(),
});
  ...
  update: tenantProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const transaction = await prisma.$transaction(async (tx) => {
        ...
        const oldAmountDelta = existingTransaction.type === TransactionType.INCOME
  ...
  delete: tenantProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        const amountDelta = existingTransaction.type === TransactionType.INCOME
          ? -Number(existingTransaction.amount)
          : Number(existingTransaction.amount);
